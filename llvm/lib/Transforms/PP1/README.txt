1. Download and install the sources for LLVM 9 or 10.
...
3. Unpack the PA1 tar file into a new directory somewhere
   in the LLVM source tree, e.g., lib/Transforms/Scalar/PP1/.

4. Add the following line in CMakeLists.txt in the parent directory of PP1:
	add_subdirectory(PP1)

5. Go to $LLVMOBJROOT and run 'make'.
   This should complete successfully and produce a loadable
   module $LLVMOBJROOT/lib/LLVMPA4.so.  You can run our LICM
   with:
	opt -load $LLVMOBJROOT/lib/LLVMPP1.so <other_passes> \
	    -pp1-scalarreplace <more_passes> input.bc -o output.bc
